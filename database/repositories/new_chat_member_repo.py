...
class NewChatMemberRepo(BaseRepo[NewChatMember]):
    ...
    async def pop_old_records(self):
        current_datetime = datetime.now()
        select_smth = (
            select(NewChatMember)
            .filter(NewChatMember.restriction_datetime < current_datetime)
        )

        entities = (await self.session.execute(select_smth)).scalars().all()
